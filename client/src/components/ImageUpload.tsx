      ...
      <div style={{ display:"flex", gap:"0.75rem", flexWrap:"wrap", justifyContent:"center", width:"100%" }}>
        {[
          ...
          <div key={item.label} style={{
            ...
            <div>
              <div style={{ fontSize:"0.65rem", color:"rgba(255,255,255,0.5)", fontWeight:600, letterSpacing:"0.8px" }}>{item.label}</div>
              <div style={{ fontSize:"1.1rem", fontWeight:800, color:"#fff" }}>{item.pct.toFixed(1)}%</div>
            </div>
          </div>
        ))}
      </div>
...
export default function ImageUpload() {
  ...
  const validate = (f: File): { valid: boolean; error?: string } => {
    ...
    if (!types.includes(f.type.toLowerCase())) return { valid:false, error:"Only JPG, PNG, WebP allowed" };
    if (f.size > 10*1024*1024) return { valid:false, error:"Max file size is 10 MB" };
  ...
  const handleFile = (f: File) => {
    ...
    cleanup();
    setFile(f); setError(""); setResult(null); setMetadata(null);
    const reader = new FileReader();
    fileReaderRef.current = reader;
    reader.onloadend = () => { if (typeof reader.result === "string") setPreview(reader.result); };
    reader.onerror = () => { setError("Failed to read file"); fileReaderRef.current = null; };
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (f) handleFile(f); else setError("Please upload a valid image file");
  };

  const handleImageLoad = () => {
    if (imgRef.current && file) {
      setMetadata({
        dimensions: { width: imgRef.current.naturalWidth, height: imgRef.current.naturalHeight },
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(res.status === 429 ? "Too many requests. Try again later." : `Request failed: ${res.status}`);
  ...
  const handleReset = () => {
    ...
    setError(""); setMetadata(null); lastSubmitRef.current = 0;
  };
      ...
      <style jsx>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .analyze-btn:hover:not(:disabled) {
          transform: translateY(-2px);
          box-shadow: 0 12px 32px rgba(0,245,160,0.28);
        }
        ...
        .spinner {
          ...
          border: 3px solid rgba(7,8,26,0.3);
        ...
        <div className="card">
          ...
          >
            ...
            <label htmlFor="file-upload" className="upload-label">
              ...
              <div>
                ...
                <div className="upload-hint">JPG · PNG · WebP &nbsp;·&nbsp; Max 10 MB &nbsp;·&nbsp; Secure Processing</div>
              </div>
            </label>
          </div>
          ...
          {preview && !result && (
            <button className="analyze-btn" onClick={handleSubmit} disabled={!file || loading}>
              {loading
                ? <><div className="spinner" /><span>Scanning Image…</span></>
            ...
            <>
              <div className="content-grid">
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:"0.75rem" }}>
                  <div className="section-label"><Upload size={18} />Uploaded Image</div>
                  <div className="preview-box">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img ref={imgRef} src={preview} alt="Preview"
                      className="image-preview" onLoad={handleImageLoad} />
                    {metadata && (
                      <div className="image-meta">
                        <div className="meta-item"><Maximize2 size={12} />{metadata.dimensions.width}×{metadata.dimensions.height}</div>
                        <div className="meta-item"><ImageIcon size={12} />{metadata.type}</div>
                        <div className="meta-item"><span className="meta-badge">{metadata.size}</span></div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {result && (
                <div className="result-footer">
                  <div className={`result-badge ${result.label === "real" ? "badge-real" : "badge-fake"}`}>
                    {result.label === "real"
                      ? <><CheckCircle size={16} />Likely Real Image</>
                      : <><AlertCircle size={16} />Likely Fake Image</>}
                  </div>
          ...
          {!preview && (
            <div className="security-note">
              <Shield size={14} />
              <span>Images are processed securely and not stored after analysis</span>
            </div>
          )}
